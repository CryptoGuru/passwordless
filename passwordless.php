<?php
/*
Plugin Name:  Password Less Sign-In with Device Authentication
Version: 1.1
Plugin URI: http://devauth.com/wordpress/passwordless/
Description: Allows users to sign up and log in using only their email addresses only from their computer and browser.
Author: Sorin Iclanzan, Gurudatt Shenoy
Author URI: http://hel.io/, http://devauth.com
License: GPL3
Text Domain: passwordless, device authentication
Domain Path: /languages
*/

/*
	Copyright 2012 Sorin Iclanzan  (email : sorin@hel.io)
	Copyright 2013 Gurudatt Shenoy  (email : gurudatt@easysecured.com)
	
	This file is part of  Passwordless Authentication
	...
	along with  Passwordless Authentication. If not, see http://www.gnu.org/licenses/gpl.html.
*/

require_once('easysecured.php');
include_once('mobiled.php');
...
class Passwordless extends Skeleton {
	...
	public function login_page() {
		...
		if ( isset( $_REQUEST['key'] ) ) {
			$user_id = $this->login(); // after login
		}
	...
	public function request_login( $user_email ) {
		...
		if ( ! $user_id = email_exists( $user_email ) ) {
			...
			// disable the following two lines if you want to enable new user registration.
			$this->front_notices->add( 'registerdisabled', __( 'User registration is currently not allowed.' ) );
			return false;

			/* // New user registration has been disabled. Remove this to enable it.
		...
		$browser = preg_replace('/[^\w\d_ -]/si', '', $browser);
		...
		$deviceid=$browser.$mtype.$lang;

		// locking key to device and browser
		
		$ekey = mencrypt($key,$deviceid);
		
		$login_link = add_query_arg( 'key', $ekey ."*==*".$user_id, home_url( $this->login_page ) );
	...
	protected function login() {
		...
		$dkey = mdecrypt($key,$deviceid);
				
		$hash = get_transient( 'login_hash_' . $user_id );

		if ( ! $hash || ! wp_check_password( $dkey, $hash ) ) {
			$this->front_notices->add( 'invalid_key', __( 'Invalid key' ) );
			return 0;
		}
		$secure_cookie = '';
